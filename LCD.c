...
void LCD_Init(void)
{
    ...
    __delay_us(40);
    LCD_sendnibble(0b0010); //function set (sent as nibble as not yet set as 4 bit mode)
    __delay_us(40);
    LCD_sendbyte(0b00101000,0); //function set
    __delay_us(40);
    LCD_sendbyte(0b00101000,0); //function set
    __delay_us(40);
    LCD_sendbyte(0b00001100,0); //set display, cursor and blinking cursor on/off with last 3 bits
    __delay_us(40);
    LCD_sendbyte(0b00000001,0); //display clear
    __delay_ms(2);
    LCD_sendbyte(0b00000110,0); //set entry mode
...
{
    LCD_sendbyte(0b00000001,0); //Clear display every time function is called
    __delay_ms(2);
...
void LCD_scroll(void)
{
    ...
    LCD_sendbyte(0b00011000,0); //function set
    __delay_us(40);
...
void ADC2String(char *buf, unsigned int ADC_val){
    ...
    sprintf(buf,"v=%d.%02d",int_part,frac_part); //sprintf is function that prints formatted text into a string (p.389) for printf function
    //See p.391 for explanation of '%d', where d specifies that 'ADC_val' is signed int
}
